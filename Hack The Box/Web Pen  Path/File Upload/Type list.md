...
![Profile image update prompt with upload button, placeholder image, only images allowed.](https://cdn.services-k8s.prod.aws.htb.systems/content/modules/136/file_uploads_content_type_upload.jpg)
...
```shell-session
underhill341@htb[/htb]$ wget https://raw.githubusercontent.com/danielmiessler/SecLists/refs/heads/master/Discovery/Web-Content/web-all-content-types.txt
underhill341@htb[/htb]$ cat web-all-content-types.txt | grep 'image/' > image-content-types.txt
```
...
![POST request to /upload.php with file 'shell.php' disguised as image.](https://cdn.services-k8s.prod.aws.htb.systems/content/modules/136/file_uploads_bypass_content_type_request.jpg)
...
```shell-session
underhill341@htb[/htb]$ echo "this is a text file" > text.jpg 
underhill341@htb[/htb]$ file text.jpg 
text.jpg: ASCII text
```
...
```shell-session
underhill341@htb[/htb]$ echo "GIF8" > text.jpg 
underhill341@htb[/htb]$ file text.jpg
...
![POST request to /upload.php with file 'shell.php' disguised as image.](https://cdn.services-k8s.prod.aws.htb.systems/content/modules/136/file_uploads_bypass_mime_type_request.jpg)

This time we get `File successfully uploaded`, and our file is successfully uploaded to the server:

![Profile image update prompt with upload button, placeholder image, file successfully uploaded.](https://cdn.services-k8s.prod.aws.htb.systems/content/modules/136/file_uploads_bypass_mime_type.jpg)

We can now visit our uploaded file, and we will see that we can successfully execute system commands:

![GIF8 header with UID, GID, and groups set to 33 (www-data).](https://cdn.services-k8s.prod.aws.htb.systems/content/modules/136/file_uploads_php_manual_shell_gif.jpg)
